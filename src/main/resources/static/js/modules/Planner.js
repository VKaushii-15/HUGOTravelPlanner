...
/*const apiKey = '919e4dbaa31619516829326dd899ac76';
...
export const getDates = () => {
  const datesArray = [];
  try{
    const startDatevalue = new Date(elements.startDateInput.value.replace(/-/g, '\/'));
    const endDatevalue = new Date(elements.endDateInput.value.replace(/-/g, '\/'));
    ...
    while(startDate <= endDate) {
      ...
      throw error; // Re-throw the error to be caught in controlInit
    }
    return datesArray;
  };
...
export const showDailyPlanners = (dateRange) => {
  /*const weatherData = {};
